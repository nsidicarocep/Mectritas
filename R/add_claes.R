...
# Armar funcion
add_claes <- function(data, agregacion_deseada = c('clae6','clae3','clae2','letra'),descripciones=T) {
  # Seleccionar agregacion actual de los datos 
  posibles_agregaciones <- c('clae6','clae3','clae2','letra')
  agregacion_actual <- colnames(data)
  agregacion_actual <- agregacion_actual[agregacion_actual %in% posibles_agregaciones]
  ...
  if(length(agregacion_actual) > 0){
    ...
    if (descripciones ==T) {
      dicc_sector_tmp <- unique(dplyr::select(DatosAbiertosCEP::dicc_sector,dplyr::starts_with(agregacion_deseada)))
    } else {
      dicc_sector_tmp <- unique(dplyr::select(DatosAbiertosCEP::dicc_sector,dplyr::starts_with(agregacion_deseada)))
      dicc_sector_tmp <- dplyr::select(DatosAbiertosCEP::dicc_sector,-dplyr::contains('desc'))
    }
    
    #Joinear con los datos seleccionados 
    data <- merge.data.table(data,dicc_sector_tmp,by=agregacion_actual,all.x=T,sort = F)
